...
class PlantDashboardCard extends HTMLElement {
    ...
    if (!this._unsub && this._hass && this._hass.connection) {
      try {
        this._unsub = this._hass.connection.subscribeEvents((ev) => {
          ...
          if (!eid) return;
          if (eid.startsWith('plant.') || eid.includes('watering') || eid.startsWith('sensor.')) {
  ...
  async _render() {
    ...
    filtered.forEach(({plantState, plantInfo, wateringEntity, wateringState}) => {
      ...
      img.setAttribute('alt', plantState.attributes.friendly_name || plantState.entity_id);
      ...
      const btnDone = document.createElement('button'); btnDone.innerHTML = '✅ Done'; btnDone.setAttribute('aria-label', 'Mark watered');
      ...
      const btnSnooze = document.createElement('button'); btnSnooze.innerHTML = '⏱️ Snooze 1h'; btnSnooze.setAttribute('aria-label', 'Snooze 1 hour');
  ...
  getCardSize() {
    const visible = this.config.show_all ? Object.values(this._hass.states).filter(s=>s.entity_id.startsWith('plant.')).length : (this.config.entities||[]).length;
